...
export const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
  ...
  return (
    <aside
      className={`relative flex flex-col h-full bg-[var(--sidebar-background)] border-r border-[var(--sidebar-border)] text-[var(--sidebar-foreground)] transition-all duration-300 select-none ${
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 h-16 border-b border-[var(--sidebar-border)] shrink-0">
        ...
        {!isCollapsed && (
          <button
            onClick={onToggle}
            className="w-7 h-7 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors shadow-xs ml-2"
      ...
      {isCollapsed && tooltip && (
        <div
          ...
          className="fixed left-20 ml-2 -translate-y-1/2 px-3 py-1.5 bg-slate-900 dark:bg-slate-800 text-white text-xs font-medium rounded-lg shadow-2xl z-50 pointer-events-none whitespace-nowrap border border-slate-700 animate-in fade-in zoom-in-95 duration-150"
        >
          {tooltip.label}
        </div>
      )}
